...
export const CURATED_NAMES: Set<string> = new Set(
  CURATED_COMBOS.map((c) => comboName(c)),
);
...
// Social tools — every ally needs the sending stone and notes to communicate.
// These are appended to every job's tool whitelist.
const ALLY_SOCIAL_TOOLS = "stone_send,stone_receive,write_notes";

export const JOB_TOOLS: Record<Job, string> = {
  scout: `read,grep,find,ls,bash,${ALLY_SOCIAL_TOOLS}`,
  reviewer: `read,grep,find,ls,bash,${ALLY_SOCIAL_TOOLS}`,
  coder: `read,grep,find,ls,bash,write,edit,${ALLY_SOCIAL_TOOLS}`,
  researcher: `read,grep,find,ls,bash,${ALLY_SOCIAL_TOOLS}`,
  planner: `read,grep,find,ls,${ALLY_SOCIAL_TOOLS}`,
};

export const JOB_DEFAULTS: Record<
  Job,
  { timeoutMs: number; checkInIntervalMs: number }
> = {
