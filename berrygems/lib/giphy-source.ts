...
import { readPantrySetting } from "./settings.ts";
...
export async function searchGiphy(query: string): Promise<string | null> {
  try {
    const rating = readPantrySetting<string>(
      "todos.gifRating",
      DEFAULT_GIF_RATING,
    );
    const validRating = VALID_RATINGS.includes(rating)
      ? rating
      : DEFAULT_GIF_RATING;
...
export async function fetchGiphyImage(
  ...
  const dims = getGifDimensions(gifBuffer.toString("base64")) ?? {
    widthPx: 100,
    heightPx: 100,
  };
  return {
  ...
  if (source.startsWith("http://") || source.startsWith("https://")) {
    ...
    try {
      const resolved = source.startsWith("~")
        ? join(process.env.HOME ?? "", source.slice(1))
        : source;
...
function getVibePrompt(): string {
  return readPantrySetting<string>("todos.gifVibePrompt", DEFAULT_VIBE_PROMPT);
}
