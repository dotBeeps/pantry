...
//
// Pantry's own features don't gate through dragon-lab — they ship on by default.
...
const ANTHROPIC_CONTEXT_MANAGEMENT: LabFeature = {
  ...
  provider: "anthropic",
  description:
    "Server-side context editing: clear_tool_uses, clear_thinking, compact_20260112",
};
...
export default function dragonLab(pi: ExtensionAPI): void {
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (model.provider === "anthropic") {
      const enabled = readPantrySetting<boolean>(
        "lab.anthropic.contextManagement",
        true,
      );
      if (enabled !== false) {
        ...
        _active.set(
          ANTHROPIC_CONTEXT_MANAGEMENT.id,
