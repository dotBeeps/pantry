...
function toggleTodoStatus(cwd: string, todoId: string): TodoFile | null {
  ...
  writeFileSync(
    fp,
    JSON.stringify(meta, null, 2) + content.slice(endBrace + 1),
    "utf-8",
  );
...
class TodoPanelComponent {
  ...
  refresh(): void {
    ...
    if (this.selectedIndex >= this.todos.length)
      this.selectedIndex = Math.max(0, this.todos.length - 1);
    this.invalidate();
  }
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) && this.selectedIndex > 0) {
      ...
      if (todo) {
        toggleTodoStatus(this.cwd, todo.id);
        this.refresh();
        this.tui.requestRender();
      }
    }
  ...
  render(width: number): string[] {
    ...
    if (this.todos.length === 0) {
      ...
      if (barWidth >= 5) {
        const filled =
          totalCount > 0 ? Math.round((doneCount / totalCount) * barWidth) : 0;
        const pct =
          totalCount > 0 ? Math.round((doneCount / totalCount) * 100) : 0;
        pushLine(
          "  " +
            th.fg("success", "█".repeat(filled)) +
            th.fg("dim", "░".repeat(barWidth - filled)) +
            th.fg("muted", ` ${pct}%`),
        );
        pushLine("");
      }
      ...
      const visibleStart = Math.max(
        0,
        Math.min(this.scrollOffset, this.todos.length - maxVisible),
      );
      ...
      for (let i = 0; i < visibleEnd - visibleStart; i++) {
        ...
        const check =
          todo.status === "done" ? th.fg("success", "✓") : th.fg("dim", "○");
        const pointer = isSelected ? th.fg("accent", "▸ ") : "  ";
        const titleColor =
          todo.status === "done"
            ? th.fg("muted", th.strikethrough(todo.title))
            : isSelected
              ? th.fg("accent", todo.title)
              : th.fg("text", todo.title);
        pushLine(`${pointer}${check} ${titleColor}`);
        if (isSelected && todo.body) {
          const preview = todo.body.split("\n")[0] ?? "";
          if (preview.trim())
            pushLine(
              "     " + th.fg("dim", truncateToWidth(preview, innerW - 7)),
            );
        }
      }

      if (visibleEnd < this.todos.length)
        pushLine(th.fg("dim", `  ↓ ${this.todos.length - visibleEnd} more`));
      pushLine("");
    }
    ...
    const help = focused
      ? th.fg(
          "dim",
          `↑↓ nav · Space toggle · ${kh?.focused ?? "Q close · Escape unfocus"}${focusCounter}`,
        )
      : th.fg(
          "dim",
          `${kh?.unfocused ?? "Ctrl+Arrows focus"} · /todos help${focusCounter}`,
        );
...
const TodoPanelParams = Type.Object({
  ...
  tag: Type.Optional(
    Type.String({
      description: "Tag to filter todos by (use 'all' for all todos)",
    }),
  ),
  anchor: Type.Optional(
    Type.String({
      description:
        "Panel position: top-left, top-center, top-right, left-center, center, right-center, bottom-left, bottom-center, bottom-right",
    }),
  ),
  relativeTo: Type.Optional(
    Type.String({
      description:
        "Anchor relative to another panel's edge. Panel ID to anchor to (use with relativeEdge)",
    }),
  ),
  relativeEdge: Type.Optional(
    Type.String({
      description:
        "Edge of the reference panel to anchor to: top, bottom, left, right, top-left, top-right, bottom-left, bottom-right",
    }),
  ),
  width: Type.Optional(
    Type.String({
      description: "Panel width as number or percentage (e.g. '30%' or '40')",
    }),
  ),
  count: Type.Optional(
    Type.Number({ description: "Number of panels for suggest_layout" }),
  ),
  offsetX: Type.Optional(
    Type.Number({ description: "Horizontal offset from anchor position" }),
  ),
  offsetY: Type.Optional(
    Type.Number({ description: "Vertical offset from anchor position" }),
  ),
  focus: Type.Optional(
    Type.Boolean({
      description:
        "If true, immediately focus this panel after opening. Default: false",
    }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  if (process.env["PANTRY_GUARD_MODE"] === "ally") return;
  ...
  function panelId(tag: string): string {
    return `todo:${tag}`;
  }
  ...
  pi.on("session_switch" as any, async () => {
    todoComponents.clear();
  });
  pi.on("session_shutdown" as any, async () => {
    todoComponents.clear();
  });
  pi.on("tool_result", async (event) => {
    if (event.toolName === "todo" && todoComponents.size > 0)
      refreshAllPanels();
  });

  // ── Tool ──
  const makeResult = (text: string, error?: boolean) => ({
    content: [{ type: "text" as const, text }],
    details: { panelCount: todoComponents.size, error },
  });
  ...
  pi.registerTool({
    ...
    label: "Todo Panel",
    description:
      "Manage floating todo panels. Panels display todos from .pi/todos filtered by tag. Actions: open (tag, anchor?, width?), close (tag), close_all, focus (tag?), unfocus, list_panels, suggest_layout (count), refresh. Use the built-in 'todo' tool for CRUD operations on todos.",
    promptSnippet:
      "Open/close/focus persistent floating todo panels showing .pi/todos filtered by tag",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      if (!ctx.hasUI && params.action !== "suggest_layout")
        return makeResult("Error: panels require interactive mode", true);
      switch (params.action) {
        case "open":
          return params.tag
            ...
            : makeResult("Error: tag required for open", true);
        case "close":
          return params.tag
            ? makeResult(closePanel(params.tag))
            : makeResult("Error: tag required for close", true);
        case "close_all":
          return makeResult(closeAllTodoPanels());
        case "focus": {
          const p = getPanels();
          return makeResult(
            params.tag
              ? (p?.focusPanel(panelId(params.tag)) ??
                  "Panel manager unavailable")
              : (p?.focusDirection?.("right") ?? "Panel manager unavailable"),
          );
        }
        case "unfocus":
          getPanels()?.unfocusAll();
          return makeResult("All panels unfocused");
        case "list_panels":
          return makeResult(listPanels());
        case "suggest_layout":
          return makeResult(
            getSuggestedLayout(params.count ?? todoComponents.size + 1),
          );
        case "refresh":
          refreshAllPanels();
          return makeResult("Refreshed all panels");
        default:
    ...
    renderCall(args, theme) {
      let text =
        theme.fg("toolTitle", theme.bold("todo_panel ")) +
    ...
    renderResult(result, _options, theme) {
      const details = result.details as
        | { panelCount?: number; error?: boolean }
        | undefined;
      const msg =
      ...
      const info =
        details?.panelCount !== undefined
          ? theme.fg("dim", ` (${details.panelCount} panel(s))`)
          : "";
      return new Text(
        theme.fg("success", "✓ ") + theme.fg("muted", msg) + info,
        0,
        0,
      );
    },
    ...
    handler: async (args, ctx) => {
      ...
      switch (subcmd) {
        case "open": {
          ...
          if (!tag) {
            ctx.ui.notify(
              "Usage: /todos open <tag> [anchor] [width]",
              "warning",
            );
            return;
          }
          ...
          const sizeArg = parts
            .slice(2)
            .find((p) => p.toLowerCase() in IMAGE_SIZES);
          const posArgs = parts
            .slice(2)
            .filter((p) => !(p.toLowerCase() in IMAGE_SIZES));
          ctx.ui.notify(
            openPanel(
              tag,
              posArgs[0],
        ...
        case "close": {
          ...
          if (!tag) {
            ...
            for (const [t] of todoComponents) {
              if (focId === panelId(t)) {
                ctx.ui.notify(closePanel(t), "info");
                return;
              }
            }
            ctx.ui.notify("No focused todo panel to close", "warning");
            return;
          }
          ctx.ui.notify(closePanel(tag), "info");
          return;
        }
        case "close-all":
          ctx.ui.notify(closeAllTodoPanels(), "info");
          return;
        case "focus": {
          if (todoComponents.size === 0) {
            ctx.ui.notify("No todo panels open", "info");
            return;
          }
          const fp = getPanels();
          ctx.ui.notify(
            parts[1]
              ? (fp?.focusPanel(panelId(parts[1])) ??
                  "Panel manager unavailable")
              : (fp?.focusDirection?.("right") ?? "Panel manager unavailable"),
            "info",
          );
          return;
        }
        case "layout": {
          const c = parts[1] ? parseInt(parts[1], 10) : todoComponents.size + 1;
          ctx.ui.notify(getSuggestedLayout(isNaN(c) ? 1 : c), "info");
          return;
        }
        case "status":
          ctx.ui.notify(listPanels(), "info");
          return;
        case "refresh":
          refreshAllPanels();
          ctx.ui.notify("Refreshed all panels", "info");
          return;
        default:
          ctx.ui.notify(
            [
