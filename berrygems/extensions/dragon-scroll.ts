...
const PopupParams = Type.Object({
  title: Type.Optional(
    Type.String({ description: "Panel title (shown in header)" }),
  ),
  ...
  width: Type.Optional(
    Type.String({
      description:
        "Panel width as number or percentage, e.g. '50%' or '60'. Default: 50%",
    }),
  ),
  id: Type.Optional(
    Type.String({
      description: "Panel ID for updates/closing. Default: auto-generated",
    }),
  ),
  skin: Type.Optional(
    Type.String({
      description:
        "Panel skin name (e.g. 'ember', 'curvy', 'box'). Default: from settings",
    }),
  ),
});
...
function extractInlineImages(content: string): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (pipeIdx !== -1) {
      ...
      const qualifiers = url!
        .slice(pipeIdx + 1)
        .split("|")
        .map((q) => q.trim().toLowerCase());
      for (const q of qualifiers) {
        if (SIZE_QUALIFIERS.includes(q)) size = q;
        else if (FLOAT_QUALIFIERS.includes(q as FloatMode))
          float = q as FloatMode;
      }
    }
...
class PopupComponent {
  ...
  private async loadInlineImage(
    ...
    if (!imageData) {
      ...
      imageData = imageFetch
        ? ((await imageFetch.fetch(source)) ?? undefined)
        : undefined;
      if (!imageData) return;
  ...
  private expandImageMarkers(allLines: string[], innerW: number): string[] {
    ...
    const markerRe = new RegExp(
      `${escapeRegex(IMG_MARKER_PREFIX)}(\\d+)${escapeRegex(IMG_MARKER_SUFFIX)}`,
    );
    ...
    while (i < allLines.length) {
      ...
      if (!entry.player) {
        ...
        result.push(
          theme.fg(
            "dim",
            `⏳ Loading image${entry.ref.alt ? `: ${entry.ref.alt}` : ""}...`,
          ),
        );
      ...
      const imgLine = (content: string) =>
        IMG_LINE_TAG + innerW + IMG_WIDTH_SEP + content;
      ...
      if (float === "center") {
        ...
        for (const pLine of placeholderLines) {
          result.push(
            imgLine(" ".repeat(leftPad) + pLine + " ".repeat(rightPad)),
          );
        }
        ...
        for (let pr = 0; pr < placeholderLines.length; pr++) {
          if (i < allLines.length && !isMarkerLine(allLines[i]!)) {
            const [leftPart, rightPart, leftVisible] = splitAtWordBoundary(
              allLines[i]!,
              leftW,
            );
  ...
  render(width: number): string[] {
    ...
    if (this.renderedLines.length === 0) {
      ...
      const mdContent =
        this.inlineImages.length > 0 ? this.processedContent : this.content;
    ...
    const contentLines = visible.map((line) => {
      if (line.startsWith(IMG_LINE_TAG)) {
        ...
        const imgContent =
          sepIdx !== -1
            ? line.slice(sepIdx + IMG_WIDTH_SEP.length)
            : line.slice(IMG_LINE_TAG.length);
        const padding = Math.max(0, innerW - visibleWidth(imgContent));
        return bgWrap(
          edges.left + imgContent + " ".repeat(padding) + edges.right,
        );
      }
    ...
    const focusCounter = focusPos
      ? ` · ${focusPos.index}/${focusPos.total}`
      : "";
    if (total > viewH) {
...
export default function popup(pi: ExtensionAPI): void {
    ...
    async execute(
      ...
      if (!panels) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Panel manager not available \u2014 dragon-parchment extension required",
            },
          ],
          details: {},
        };
      ...
      if (existing) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Updated popup "${params.title ?? id}"`,
            },
          ],
      ...
      if (!result.success) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Failed to open popup: ${result.message}`,
            },
          ],
          details: {},
        };
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Opened popup "${params.title ?? id}"`,
          },
        ],
  ...
  pi.registerTool({
    ...
    promptSnippet: "Close a popup panel by ID or close all popups",
    parameters: Type.Object({
      id: Type.Optional(
        Type.String({
          description: "Panel ID to close. Omit to close all popups.",
        }),
      ),
    }),
    ...
    async execute(
      ...
      if (!panels) {
        return {
          content: [
            { type: "text" as const, text: "Panel manager not available" },
          ],
          details: {},
        };
      }
      ...
      if (params.id) {
        if (!activePopups.has(params.id)) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `No popup with id "${params.id}". Active: ${available.join(", ") || "(none)"}`,
              },
            ],
            details: {},
          };
        ...
        return {
          content: [
            { type: "text" as const, text: `Closed popup: ${params.id}` },
          ],
    ...
    renderResult(result, _options, theme, _context) {
      ...
      if (d?.action === "closed-all") {
        return new Text(
          theme.fg("muted", `🧹 cleared ${d.count} popup(s)`),
          0,
          0,
        );
      }
