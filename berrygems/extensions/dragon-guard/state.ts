...
import {
  DOG_MODE_ALLOWED_TOOLS,
  PUPPY_MODE_DEFAULT_ALLOWED_TOOLS,
} from "./settings.ts";
...
export function getDogModeToolPolicy(
  toolName: string,
): "allow" | "block" | "prompt" {
  if (dogModeSessionBlockedTools.has(toolName)) return "block";
  if (
    DOG_MODE_ALLOWED_TOOLS.has(toolName) ||
    dogModeSessionAllowedTools.has(toolName)
  )
    return "allow";
  return "prompt";
}

export function getPuppyModeToolPolicy(
  toolName: string,
  input: unknown,
): "allow" | "prompt" {
...
export function reconstructState(entries: readonly any[]): GuardMode {
  ...
  for (const entry of entries) {
    if (entry.type !== "custom" || entry.customType !== "dragon-guard-state")
      continue;
    ...
    for (const toolName of data?.dogModeSessionBlockedTools ?? []) {
      if (typeof toolName === "string")
        dogModeSessionBlockedTools.add(toolName);
    }
