 ...
 * panel registered with dragon-parchment. Shows current mode, live-tweakable
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.id === "reset") {
        ...
        if (mode === "dragon") {
          ...
          const allowed =
            [...puppyModeSessionAllowedTools].sort().join(", ") || "(defaults)";
          add(
            th.fg("dim", "  Session allows: ") +
              th.fg("text", truncateToWidth(allowed, Math.max(4, width - 20))),
          );
        } else {
          const allowed =
            [...dogModeSessionAllowedTools].sort().join(", ") || "(none)";
          const blocked =
            [...dogModeSessionBlockedTools].sort().join(", ") || "(none)";
          add(
            th.fg("dim", "  + allowed: ") +
              th.fg(
                "success",
                truncateToWidth(allowed, Math.max(4, width - 16)),
              ),
          );
          add(
            th.fg("dim", "  − blocked: ") +
              th.fg("error", truncateToWidth(blocked, Math.max(4, width - 16))),
          );
        }
      ...
      switch (item.id) {
        case "mode": {
          ...
          const modeColor =
            mode === "dragon" ? "warning" : mode === "plan" ? "accent" : "text";
          valueStr =
            th.fg("muted", "◂ ") +
            th.fg(modeColor, modeLabel) +
            th.fg("muted", " ▸");
          break;
        }
        case "auto-detect":
          valueStr = getAutoDetect()
            ? th.fg("success", "● ON")
            : th.fg("error", "○ OFF");
          break;
        case "sensitivity":
          valueStr =
            th.fg("muted", "◂ ") +
            th.fg("text", String(getComplexityThreshold())) +
            th.fg("muted", " ▸");
          break;
        case "llm-summaries":
          valueStr = getLlmSummaries()
            ? th.fg("success", "● ON")
            : th.fg("error", "○ OFF");
          break;
        case "reset":
