...
import {
  ...
  dogModeSessionBlockedTools,
  puppyModeSessionAllowedTools,
  MODE_LABEL,
  type GuardMode,
} from "./state.ts";
...
function fallbackToolSummary(toolName: string): string {
  ...
  if (toolName === "bash")
    return "Run a shell command that may read, modify, or affect system/project state.";
  if (toolName === "edit") return "Modify a file by replacing exact text.";
  if (toolName === "write")
    return "Create or overwrite a file with provided content.";
...
export default function dragonGuardExtension(pi: ExtensionAPI): void {
  ...
  function renderDragonStatus(now: number): string {
    ...
    const breathe =
      0.72 + (0.22 * (Math.sin((now / 2600) * 2 * Math.PI) + 1)) / 2;
    let out = "";
  ...
  function renderPuppyStatus(now: number): string {
    ...
    const breathe =
      0.82 + (0.16 * (Math.sin((now / 2500) * 2 * Math.PI) + 1)) / 2;
  ...
  function refreshModeUi(): void {
    ...
    const renderTick = () => {
      ...
      if (getMode() === "dragon") {
        ...
        uiCtx.ui.setWidget("dragon-guard", [
          theme
            ? theme.fg("success", "all edits allowed")
            : rgb("all edits allowed", 255, 170, 240),
        ]);
        return;
      }
      ...
      if (getMode() === "plan") {
        ...
        uiCtx.ui.setWidget("dragon-guard", [
          theme
            ? theme.fg("accent", "read-only planning enabled")
            : rgb("read-only planning enabled", 130, 210, 255),
        ]);
      }
    };
  ...
  async function summarizeToolUse(
    toolName: string,
    input: unknown,
    ctx: ExtensionContext,
  ): Promise<string> {
  ...
  async function askDogModePermission(
    ...
    if (choice?.startsWith("Always allow")) {
      ...
      if (ctx.hasUI) {
        ctx.ui.notify(
          `Dog Mode: always allowing ${toolName} for this session.`,
          "info",
        );
      }
      return "allow-always";
  ...
  async function askPuppyModePermission(
    ...
    const choice = await ctx.ui.select(
      `Puppy Mode readonly guard triggered:\n${callText}\n${summary}`,
      [
        "Allow this tool call once",
    ...
    if (choice?.startsWith("Always allow")) {
      ...
      persistState();
      ctx.ui.notify(
        `Puppy Mode: always allowing ${toolName} for this session.`,
        "info",
      );
      return "allow-always";
    }
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    if (
      ...
      shouldAutoPlan(promptText)
    ) {
      setMode("plan", ctx);
      if (ctx.hasUI) {
        ctx.ui.notify(
          "Auto-entered Puppy Mode for a higher-complexity request.",
          "info",
        );
      }
    }
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (getMode() === "plan") {
      ...
      const decision = await askPuppyModePermission(
        event.toolName,
        event.input,
        ctx,
      );
    ...
    if (getMode() === "none") {
      ...
      const decision = await askDogModePermission(
        event.toolName,
        event.input,
        ctx,
      );
