...
function runFallback(
  cwd: string,
  fallback: FallbackConfig,
): { diagnostics: Diagnostic[]; duration: number } {
  const start = Date.now();
  let output = "";

  // For Go, run in the directory with go.mod
  const execCwd =
    fallback.name === "go vet" ? resolve(cwd, "dragon-daemon") : cwd;
  ...
  return {
    diagnostics: fallback.parser(output, execCwd),
...
class LspManager {
  ...
  get ready(): boolean {
    return this._readyServers.size > 0;
  }
  get starting(): boolean {
    return this._startingServers.size > 0;
  }
  get configs(): LanguageConfig[] {
  ...
  get errorText(): string | null {
    const errs = [...this._errors.entries()].map(
      ([name, err]) => `${name}: ${err}`,
    );
  ...
  async start(): Promise<void> {
    ...
    const startPromises = this._configs.map((config) =>
...
class LintPanelComponent {
  ...
  private rebuildFromLsp(): void {
    ...
    const oldExpanded = new Set(
      this.groups.filter((g) => g.expanded).map((g) => g.relPath),
    );
  ...
  render(width: number): string[] {
    ...
    const modeLabel =
      this.mode === "lsp" ? "LSP" : this.fallbackName || "fallback";
    const isStarting =
      !this.lsp.ready && this.lsp.starting && this.mode === "lsp";
    const statusIcon = isStarting ? "⏳" : this.totalErrors === 0 ? "✅" : "⚠️";

    const toggleLabel =
      this.mode === "lsp" ? this.fallbackName || "fallback" : "lsp";
    ...
    if (isStarting) {
      ...
      lines.push(
        padContentLine(
          th.fg("success", "  All clear! No diagnostics."),
          width,
          chromeOpts,
        ),
      );
      ...
      lines.push(
        padContentLine(th.fg("success", "  No errors!"), width, chromeOpts),
      );
      ...
      lines.push(
        padContentLine(
          th.fg(
            "dim",
            `  ${nonErrors} ${nonErrors === 1 ? "warning" : "warnings"}`,
          ),
          width,
          chromeOpts,
        ),
      ...
      for (const group of this.groups) {
        ...
        const errCount = group.diagnostics.filter(
          (d) => d.severity === "error",
        ).length;
        ...
        if (group.expanded) {
          for (const d of group.diagnostics) {
            ...
            const dPrefix =
              isDiagSelected && focused ? th.fg("accent", "  ▶ ") : "    ";
            ...
            const sourceTag =
              this.lsp.configs.length > 1 ? th.fg("dim", ` [${d.source}]`) : "";
...
const LintParams = Type.Object({
  action: StringEnum(["check", "open", "close"] as const, {
    description:
      "check: run type/lint checks and return results. open: show lint panel with live LSP diagnostics. close: hide lint panel.",
  }),
});
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (params.action === "check") {
        ...
        if (result.diagnostics.length === 0) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `✅ No diagnostics via ${result.source}${timing}`,
              },
            ],
            details: {
              action: "check",
        ...
        if (errors.length > 0) {
          lines.push(
            `⚠️ ${errors.length} error${errors.length === 1 ? "" : "s"} via ${result.source}${timing}`,
          );
        }
        if (warnings.length > 0) {
          lines.push(
            `${errors.length === 0 ? "📋" : ""} ${warnings.length} warning${warnings.length === 1 ? "" : "s"}`,
          );
        }
        lines.push("");
      ...
      if (params.action === "open") {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: success
                ? `Opened lint panel — connecting to ${serverNames || "no servers detected"}...`
          ...
          details: {
            action: "open",
            success,
            languages: langs.map((l) => l.server.name),
          },
        };
      }
      ...
      if (params.action === "close") {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: success ? "Closed lint panel" : "No lint panel open",
            },
          ],
  ...
  pi.registerCommand("lint", {
    ...
    handler: async (args, ctx) => {
      ...
      if (action === "open") {
        ...
        ctx.ui.notify(
          ok
            ? `Lint panel — ${names || "no languages detected"}`
            : "Panel manager not available",
          ok ? "info" : "warning",
        );
