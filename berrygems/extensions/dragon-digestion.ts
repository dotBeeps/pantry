...
async function generateDigestRemark(
	...
	const customPrompt = readProjectHoardSetting(cwd, "digestion.remarkPrompt", "") as string;
	let prompt = customPrompt || DEFAULT_DIGEST_REMARK_PROMPT;

	// Fill placeholders
	prompt = prompt
		.replace(/\{tokens_before\}/g, formatTokens(stats.tokensBefore))
		.replace(/\{tokens_after\}/g, stats.tokensAfter !== null ? formatTokens(stats.tokensAfter) : "unknown")
	...
	if (!resolved) {
		return STATIC_DIGESTION_REMARKS[Math.floor(Math.random() * STATIC_DIGESTION_REMARKS.length)]!;
	}
	...
	try {
		const response = await Promise.race([
			complete(
				resolved.model,
				{
					systemPrompt: "You generate a single short in-character remark. Output ONLY the remark, nothing else.",
			...
			new Promise<never>((_, reject) => setTimeout(() => reject(new Error("remark timeout")), 10_000)),
		]);
	...
	for (const p of paths) {
		try {
			...
			for (const entry of raw) {
				if (typeof entry.id === "string" && typeof entry.label === "string" && typeof entry.instructions === "string") {
					custom.push({ id: entry.id, label: entry.label, instructions: entry.instructions });
				}
			}
		} catch {
...
function getTierThresholds(summaryThreshold: number, overrides?: DigestSettingsV2["tierOverrides"]): TierThresholds {
	const clamped = Math.max(0, Math.min(100, summaryThreshold));
	const s = clamped / 100;
	return {
		alert: overrides?.alert ?? s * 0.50 * 100,
		lightPrune: overrides?.lightPrune ?? s * 0.70 * 100,
...
function getCurrentTier(usagePercent: number, thresholds: TierThresholds): TierLevel {
 ...
 */
function buildBreadcrumb(msg: Record<string, unknown>): string | Record<string, unknown>[] {
	const toolName = (msg.toolName ?? msg.name ?? "unknown") as string;
	const rawContent = typeof msg.content === "string" ? msg.content
		: Array.isArray(msg.content) ? (msg.content as Record<string, unknown>[]).map(b =>
 ...
 */
function applyHygiene(messages: Record<string, unknown>[], keepResults: number): Record<string, unknown>[] {
 ...
 */
function applyLightPrune(messages: Record<string, unknown>[], keepResults: number): Record<string, unknown>[] {
	...
	return result.map((msg, i) => {
		...
		return { ...msg, content: isArray ? [{ type: "text", text: truncated }] : truncated };
	});
}
 ...
 */
function applyHeavyPrune(messages: Record<string, unknown>[]): Record<string, unknown>[] {
	...
	const truncateAssistantBefore = assistantIndices.length > 5 ? assistantIndices.length - 5 : 0;
	...
	return result.map((msg, idx) => {
		...
		if (Array.isArray(msg.content)) {
			const truncated = (msg.content as Record<string, unknown>[]).map((block) => {
				if (block.type === "text" && typeof block.text === "string" && block.text.length > 200) {
					return { ...block, text: block.text.slice(0, 200) + " [...truncated]" };
				}
				return block;
			});
 ...
 */
async function resolveSummaryModel(ctx: ExtensionContext, preferredModelId: string): Promise<ResolvedSummaryModel | null> {
	...
	if (preferredModelId) {
		...
		if (slash > 0) {
			...
			if (found) {
				...
				if (auth.ok && auth.apiKey) return { model: found, apiKey: auth.apiKey, headers: auth.headers };
			}
		} else {
	...
	return messages.map(msg => {
		...
		const content = typeof msg.content === "string"
			...
			: Array.isArray(msg.content)
				? (msg.content as Record<string, unknown>[]).map(block => {
					if ((block.type as string) === "text") return (block.text as string) ?? "";
					if ((block.type as string) === "tool_use") return `[Tool call: ${(block.name as string) ?? "unknown"}]`;
					if ((block.type as string) === "tool_result") return `[Tool result: ${((block.content as string) ?? "").slice(0, 200)}]`;
 ...
 */
function extractFilePathsFromMessages(messages: Record<string, unknown>[]): Set<string> {
	...
	for (const msg of messages) {
		...
		if (args) {
			...
			if (typeof args.glob === "string" && !(args.glob as string).includes("*")) paths.add(args.glob as string);
		}
		// Also check assistant content blocks for tool_use with file-like args
		if (Array.isArray(msg.content)) {
			for (const block of msg.content as Record<string, unknown>[]) {
				if (block.type === "tool_use" || block.type === "toolCall") {
					const input = (block.input ?? block.args) as Record<string, unknown> | undefined;
					if (input) {
...
Respond with exactly two lines, one per question.`;
	...
	try {
		...
		const passed = lines.filter(l => l.toUpperCase().startsWith("PASS")).length;
		const failed = lines.filter(l => l.toUpperCase().startsWith("FAIL")).length;

		// Store result on the last history entry
		const lastEntry = state.compactionHistory[state.compactionHistory.length - 1];
		if (lastEntry) {
		...
		if (failed > 0) {
			const failLines = lines.filter(l => l.toUpperCase().startsWith("FAIL")).join("\n");
...
function readDigestSettingsV2(cwd: string): DigestSettingsV2 {
	...
	return {
		...base,
		tieredMode: readProjectHoardSetting(cwd, "digestion.tieredMode", DEFAULT_DIGEST_V2.tieredMode) as boolean,
		summaryThreshold: Math.max(10, Math.min(100, readProjectHoardSetting(cwd, "digestion.summaryThreshold", DEFAULT_DIGEST_V2.summaryThreshold) as number)),
		...
		tierOverrides: {
			...
			lightPrune: readProjectHoardSetting(cwd, "digestion.tierOverrides.lightPrune", undefined) as number | undefined,
			heavyPrune: readProjectHoardSetting(cwd, "digestion.tierOverrides.heavyPrune", undefined) as number | undefined,
		},
	};
}
...
type CompactionSource = "manual_panel" | "manual_tool" | "proactive_turn_end" | "unknown";
...
function createDigestState(cwd: string): DigestState {
	return {
		...
		compactionStats: { lastCompactedAt: null, tokensBefore: null, tokensAfter: null },
		compactionHistory: [],
...
class CompactionPanelComponent {
	...
	syncContextUsage(): void {
		...
		if (usage.contextWindow !== null && usage.contextWindow !== this.lastContextWindow) {
	...
	private getItems(): PanelItem[] {
		...
		const modeLabel = TRIGGER_MODE_LABELS[this.state.digestSettings.triggerMode];
	...
	private cycleTriggerMode(direction: 1 | -1): void {
		const currentIdx = TRIGGER_MODES.indexOf(this.state.digestSettings.triggerMode);
		...
		writeDigestSetting(this.cwd, "triggerMode", this.state.digestSettings.triggerMode);
		this.recalculateReserveTokens();
		this.invalidate();
		this.tui.requestRender();
	}
	...
	private cycleDigestValue(field: keyof DigestSettings, settingsKey: string, presets: number[], direction: 1 | -1): void {
		...
		(this.state.digestSettings as unknown as Record<string, unknown>)[field] = newVal;
	...
	private cycleStrategy(direction: 1 | -1): void {
		...
		const currentIdx = strategies.findIndex((s) => s.id === this.state.digestSettings.strategy);
		...
		writeDigestSetting(this.cwd, "strategy", this.state.digestSettings.strategy);
		this.invalidate();
		this.tui.requestRender();
	}
	...
	private getEffectiveTriggerTokens(): number | null {
		...
		switch (this.state.digestSettings.triggerMode) {
			...
			case "percentage":
				return Math.round(cw * (this.state.digestSettings.triggerPercentage / 100));
			case "fixed":
	...
	private adjustItem(items: PanelItem[], direction: 1 | -1): void {
		...
		switch (item.id) {
			...
			case "summaryThreshold":
				this.state.digestSettingsV2.summaryThreshold = Math.max(10, Math.min(100, cyclePreset(
					this.state.digestSettingsV2.summaryThreshold, PERCENTAGE_PRESETS, direction
				)));
				writeDigestSetting(this.cwd, "summaryThreshold", this.state.digestSettingsV2.summaryThreshold);
				this.invalidate();
				this.tui.requestRender();
				break;
			case "tierMode":
				this.state.digestSettingsV2.tieredMode = !this.state.digestSettingsV2.tieredMode;
				writeDigestSetting(this.cwd, "tieredMode", this.state.digestSettingsV2.tieredMode);
			...
			case "hygieneKeepResults": {
				...
				this.state.digestSettingsV2.hygieneKeepResults = cyclePreset(
					this.state.digestSettingsV2.hygieneKeepResults, keepPresets, direction
				);
				writeDigestSetting(this.cwd, "hygieneKeepResults", this.state.digestSettingsV2.hygieneKeepResults);
				this.invalidate();
				this.tui.requestRender();
				break;
			}
	...
	render(width: number): string[] {
		...
		if (this.state.contextUsage.tokens !== null && this.state.contextUsage.contextWindow !== null) {
			...
			if (barW >= 5) {
				...
				if (this.state.digestSettingsV2.tieredMode) {
					const thresholdsM = getTierThresholds(this.state.digestSettingsV2.summaryThreshold, this.state.digestSettingsV2.tierOverrides);
					const markers = [
					...
					for (const m of markers) {
						const pos = Math.min(barW - 1, Math.max(0, Math.round((m.pct / 100) * barW)));
						const active = pct >= m.pct;
						markerLine[pos] = active ? th.fg("warning", m.sym) : th.fg("dim", m.sym);
					}
				...
				if (this.state.digestSettingsV2.tieredMode) {
					...
					const thresholdsB = getTierThresholds(this.state.digestSettingsV2.summaryThreshold, this.state.digestSettingsV2.tierOverrides);
					const tierB = getCurrentTier(pct, thresholdsB);
					const tierColor = tierB >= 3 ? "error" : tierB >= 1 ? "warning" : "success";
				...
				add(`  Context: ${bar} ${pct}%`);
				add(th.fg("dim", `  ${formatTokens(this.state.contextUsage.tokens)} / ${formatTokens(cw)} tokens`));
		...
		if (this.state.contextUsage.contextWindow !== null) {
			if (this.state.digestSettingsV2.tieredMode) {
				const thresholdsD = getTierThresholds(this.state.digestSettingsV2.summaryThreshold, this.state.digestSettingsV2.tierOverrides);
				const cw = this.state.contextUsage.contextWindow;
				add(th.fg("dim", `  Summary at: ${formatTokens(Math.round(cw * thresholdsD.summary / 100))} (${thresholdsD.summary}%)`));
			} else {
				const cw = this.state.contextUsage.contextWindow;
		...
		if (this.state.digestSettingsV2.tieredMode && this.state.contextUsage.percent !== null) {
			const thresholdsS = getTierThresholds(this.state.digestSettingsV2.summaryThreshold, this.state.digestSettingsV2.tierOverrides);
			...
			add(`  ${th.fg(tierInfo.color as any, `Tier ${tier}: ${tierInfo.label}`)}`);
		}
		...
		if (focused && this.state.digestSettingsV2.tieredMode) {
			...
			if (s.totalMessagesFiltered > 0) {
				add(th.fg("dim", `  Filtered: ~${s.totalMessagesFiltered} msgs this session`));
			}
			if (this.state.compactionHistory.length > 0) {
				add(th.fg("dim", `  Compactions: ${this.state.compactionHistory.length} this session`));
			}
		}

		// ── Last Compaction Stats ──
		if (this.state.compactionStats.lastCompactedAt !== null) {
			const timeStr = formatRelativeTime(this.state.compactionStats.lastCompactedAt);
			let statsLine = `  Last: ${timeStr}`;
			if (this.state.compactionStats.tokensBefore !== null && this.state.compactionStats.tokensAfter !== null) {
		...
		for (let i = 0; i < items.length; i++) {
			...
			switch (item.id) {
				...
				case "triggerFixed":
					valueStr =
						th.fg("muted", "◂ ") +
						th.fg("text", formatTokens(this.state.digestSettings.triggerFixed)) +
						th.fg("muted", " ▸");
					break;
				case "keepRecentTokens":
					valueStr =
					...
					break;
				case "strategy": {
					const preset = this.state.strategies.find((s) => s.id === this.state.digestSettings.strategy) ?? this.state.strategies[0]!;
				...
				case "tierMode":
					valueStr =
						th.fg("muted", "◂ ") +
						th.fg("text", this.state.digestSettingsV2.tieredMode ? "Tiered" : "Classic") +
						th.fg("muted", " ▸");
					break;
				case "hygieneKeepResults":
					valueStr =
						th.fg("muted", "◂ ") +
						th.fg("text", String(this.state.digestSettingsV2.hygieneKeepResults)) +
						th.fg("muted", " ▸");
					break;
				case "summaryModel":
					valueStr =
...
export default function (pi: ExtensionAPI) {
	...
	function setCompactionIntent(source: CompactionSource, autoResume = false): void {
		compactionIntent = {
			source,
			autoResume,
			resumeMessage: "Resume the interrupted work from just before compaction. Continue without restarting from scratch.",
		};
	}
	...
	function shouldAutoResumeAfterCompaction(intent: CompactionIntent | null): intent is CompactionIntent {
	...
	function openPanel(ctx: ExtensionContext): string {
		...
		const result = panels.createPanel(PANEL_ID, (panelCtx: any) => {
			...
			component.triggerCompact = () => {
				...
				const strategyPreset = strategies.find((s) => s.id === digest.strategy);
	...
	pi.on("turn_end", async (_event, ctx) => {
		...
		if (digestState.digestSettingsV2.tieredMode && !digestState.taskBoundary.suggested) {
			try {
				...
				for (const entry of recentEntries) {
					if ("message" in entry && entry.message) {
						turnMessages.push(entry.message as unknown as Record<string, unknown>);
					}
				}
				const currentPaths = extractFilePathsFromMessages(turnMessages);

				if (currentPaths.size > 0) {
					digestState.taskBoundary.recentFilePaths.push(Array.from(currentPaths));
					...
					if (digestState.taskBoundary.recentFilePaths.length >= 6) {
						const recent = new Set(digestState.taskBoundary.recentFilePaths.slice(-3).flat());
						const previous = new Set(digestState.taskBoundary.recentFilePaths.slice(-6, -3).flat());
						...
						if (similarity < 0.2 && usage?.percent != null && usage.percent >= thresholds.alert) {
							ctx.hasUI && ctx.ui.notify(
		...
		const turnStrategyPreset = turnStrategies.find((s) => s.id === turnDigest.strategy);
	...
	pi.on("session_compact", async (event, ctx) => {
		...
		updateContextUsage(ctx);
		{
			...
			const pctFreed = (before && after && before > 0) ? Math.round(((before - after) / before) * 100) : 0;
			...
			const historyEntry: CompactionHistoryEntry = {
				...
				source: details?.source === "dragon-digestion-v2" ? "dragon-digestion-v2" : "pi-default",
			};
			...
			if (
				...
				const compactionEntry = (event as unknown as Record<string, unknown>).compactionEntry as Record<string, unknown> | undefined;
				const summaryText = (compactionEntry?.summary as string) ?? "";
				if (summaryText.length > 0) {
					// Run async probe — don't block the event handler
					runQualityProbe(ctxRef, summaryText, digestState).catch(() => { /* silent */ });
				}
			}

			// 🐉 Post-digestion remark (async, doesn't block)
			const v2Remarks = digestState.digestSettingsV2;
			const autoResume = shouldAutoResumeAfterCompaction(completedCompactionIntent);
			if (v2Remarks.digestRemarks && ctx.hasUI) {
				...
				const remarkPromise = generateDigestRemark(ctx, remarkStats, v2Remarks)
					.then(remark => { digestState.pendingRemark = `🐉 ${remark}`; })
					.catch(() => {});
				if (autoResume) {
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		const preparation = (event as unknown as Record<string, unknown>).preparation as Record<string, unknown> | undefined;
		if (!preparation) return; // No preparation data — let pi handle it

		const messagesToSummarize = preparation.messagesToSummarize as Record<string, unknown>[] | undefined;
		...
		if (!messagesToSummarize?.length || !firstKeptEntryId || tokensBefore === undefined) {
		...
		try {
			...
			const prompt = (v2.anchoredUpdates && previousSummary)
				? buildAnchoredUpdatePrompt(previousSummary, serialized, strategyId, strategyInstructions)
				: buildFirstCompactionPrompt(serialized, strategyId, strategyInstructions);
			...
			const response = await complete(
				resolved.model,
				{ systemPrompt: "You are a session summarizer. Follow the instructions precisely.", messages: [userMessage] },
				{ apiKey: resolved.apiKey, headers: resolved.headers, signal: AbortSignal.timeout(30_000) },
			);
			...
			ctx.hasUI && ctx.ui.notify(`🐉 Custom digestion failed, using default: ${msg}`, "warning");
			return;
		}
	});
	...
	(pi as any).on("context", async (event: any, ctx: any) => {
		...
		if (tier >= 1) {
			if (tier > digestState.lastNotifiedTier) {
				const tierNames = ["Healthy", "Alert", "Light pruning", "Heavy pruning", "Digesting"];
				ctx.hasUI && ctx.ui.notify(
		...
		if (filtered !== msgs) {
			...
			if (maskedCount > 0) {
				if (tier >= 3) digestState.tierSavings.heavyPruneMessagesFiltered += maskedCount;
				else if (tier >= 2) digestState.tierSavings.lightPruneMessagesFiltered += maskedCount;
	...
	pi.on("before_provider_request", async (event, ctx) => {
		...
		const thresholds = getTierThresholds(v2Anthropic.summaryThreshold, v2Anthropic.tierOverrides);
		const tier = getCurrentTier(usage.percent, thresholds);
		...
		if (tier >= 2) {
			edits.push({
				...
				trigger: { type: "input_tokens", value: Math.round((thresholds.lightPrune / 100) * usage.contextWindow) },
		...
		if (tier >= 4) {
			const strategies = digestState.strategies;
			const strategy = strategies.find(s => s.id === v2Anthropic.strategy) ?? strategies[0]!;
			edits.push({
				type: "compact_20260112",
				trigger: { type: "input_tokens", value: Math.round((thresholds.summary / 100) * usage.contextWindow) },
		...
		handler: async (args, ctx) => {
			...
			switch (subcmd) {
				...
				case "status": {
					...
					const strategies = digestState.strategies;
					const strategy = strategies.find((s) => s.id === digest.strategy) ?? strategies[0]!;
					...
					if (statusV2.tieredMode) {
						const thresholds = getTierThresholds(statusV2.summaryThreshold, statusV2.tierOverrides);
						const currentUsage = ctx.getContextUsage();
						const tier = currentUsage?.percent ? getCurrentTier(currentUsage.percent, thresholds) : 0;
						statusLines.push("");
						statusLines.push(`Tier mode: Tiered (Tier ${tier} active)`);
						statusLines.push(`Summary threshold: ${statusV2.summaryThreshold}%`);
						statusLines.push(`Hygiene keep: ${statusV2.hygieneKeepResults} results`);
						statusLines.push(`Tiers: Alert ${thresholds.alert.toFixed(0)}% \u00b7 Light ${thresholds.lightPrune.toFixed(0)}% \u00b7 Heavy ${thresholds.heavyPrune.toFixed(0)}% \u00b7 Summary ${thresholds.summary}%`);
					}
				...
				case "history": {
					...
					const histLines = ["\ud83d\udc09 Compaction History (this session)\n"];
					for (const entry of digestState.compactionHistory) {
						const time = formatRelativeTime(entry.timestamp);
						const saved = entry.tokensAfter !== null
						...
						const source = entry.source === "dragon-digestion-v2"
					...
					if (sv.totalMessagesFiltered > 0) {
						...
						if (sv.hygieneMessagesMasked > 0) histLines.push(`  Hygiene: ~${sv.hygieneMessagesMasked} msgs masked`);
						if (sv.lightPruneMessagesFiltered > 0) histLines.push(`  Light prune: ~${sv.lightPruneMessagesFiltered} msgs filtered`);
						if (sv.heavyPruneMessagesFiltered > 0) histLines.push(`  Heavy prune: ~${sv.heavyPruneMessagesFiltered} msgs filtered`);
					}
				...
				case "preview": {
					...
					const branch = ctx.sessionManager.getBranch();
					const messages = branch.filter((e: any) => e.type === "message").map((e: any) => e.message) as Record<string, unknown>[];

					const toolResults = messages.filter(m => m.role === "toolResult" && !m.isError);
					const errorResults = messages.filter(m => m.role === "toolResult" && m.isError);

					// Dry-run each tier
					const afterHygiene = applyHygiene(messages, v2.hygieneKeepResults);
					const hygieneMasked = messages.filter((m, i) => afterHygiene[i] !== m).length;

					const afterLight = applyLightPrune(messages, v2.hygieneKeepResults);
					const lightMasked = messages.filter((m, i) => afterLight[i] !== m).length;

					const afterHeavy = applyHeavyPrune(messages);
					const heavyMasked = messages.filter((m, i) => afterHeavy[i] !== m).length;
	...
	pi.registerTool({
		name: "dragon_digest",
		label: "Dragon Digest",
		description: "Trigger context compaction/digestion. Use when context is getting heavy (above 60% usage) and you want to free up space. Can show a preview of what will be compacted, check current status, or trigger compaction.",
		promptSnippet: "dragon_digest: Check context usage, preview tier impact, or trigger compaction",
		promptGuidelines: [
		...
		parameters: Type.Object({
			action: Type.Union([Type.Literal("preview"), Type.Literal("compact"), Type.Literal("status")], {
				description: "Action: 'preview' to show what would be compacted, 'compact' to trigger compaction, 'status' to show current tier and usage",
			}),
		}),
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			switch (params.action) {
				case "status": {
					if (!usage?.percent || !usage?.contextWindow) {
						return { content: [{ type: "text" as const, text: "No context usage data available yet." }], details: {} };
					}
					const thresholds = getTierThresholds(v2.summaryThreshold, v2.tierOverrides);
					const tier = getCurrentTier(usage.percent, thresholds);
					const tierNames = ["Healthy", "Alert", "Light pruning", "Heavy pruning", "Digesting"];
				...
				case "compact": {
					if (!ctxRef) {
						return { content: [{ type: "text" as const, text: "No active session — cannot compact." }], details: {} };
					}
					const digestSettings = digestState.digestSettings;
					const strategyPreset = digestState.strategies.find((s) => s.id === digestSettings.strategy);
					const instructions = strategyPreset?.instructions || undefined;
					pendingCompact = true;
					setCompactionIntent("manual_tool");
					ctxRef.compact({
						...
						onError: (err: Error) => {
							pendingCompact = false;
							clearCompactionIntent();
							ctx.hasUI && ctx.ui.notify(`🐉 Compaction failed: ${err.message}`, "error");
						},
					});
					return {
						content: [{ type: "text" as const, text: "Compaction triggered. The dragon is digesting — context will be compacted after the current turn." }],
						details: {},
					};
				}
				...
				default:
					return {
						content: [{ type: "text" as const, text: "Unknown action. Use 'preview', 'compact', or 'status'." }],
						details: {},
					};
			}
		},
