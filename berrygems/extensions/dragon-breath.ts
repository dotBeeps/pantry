...
import { readHoardSetting, readHoardKey } from "../lib/settings.ts";
...
function getEnabled(): boolean {
	return readHoardSetting<boolean>("breath.enabled", true);
}

function getGridIntensity(): number {
	// Direct override takes highest priority
	const override = readHoardSetting<number>("breath.gridIntensity", 0);
	if (override > 0) return override;
	// Region lookup
	const region = readHoardKey("breath.gridRegion", "default");
 ...
 */
function computeWh(inputTokens: number, outputTokens: number, whPer1KOutput: number): number {
...
const EQUIVALENCES: [number, string, string][] = [
	...
	[500,    "hour of a window air conditioner",    "hours of a window air conditioner"], // 5000 BTU unit, DOE/AHRI
	// Large
...
function topEquivalences(wh: number, count: number = 2): string[] {
	...
	for (const [unitWh, singular, plural] of EQUIVALENCES) {
		...
		const fmt = n < 1.005 && n > 0.995 ? `1 ${singular}`
			: n >= 100 ? `${Math.round(n)} ${plural}`
			: `${n.toFixed(1)} ${plural}`;
...
function renderWidget(ctx: ExtensionContext, stats: BreathStats): void {
	...
	const lastPart = stats.lastWh > 0
		? theme.fg(lastColor, `⚡ ${fmtWh(stats.lastWh)} · 🌍 ${fmtGCO2(stats.lastGCO2)}`)
		: theme.fg("dim", "⚡ —");

	const equiv = bestEquivalence(stats.sessionWh);
	const equivPart = equiv ? theme.fg("dim", ` ${equiv}`) : "";
	const sessionPart = theme.fg("dim", `(session: ${fmtWh(stats.sessionWh)} · ${fmtGCO2(stats.sessionGCO2)}${equivPart})`);
...
function handleCarbonCommand(ctx: ExtensionContext, stats: BreathStats): void {
	const gridIntensity = getGridIntensity();
	const region = readHoardKey("breath.gridRegion", "default") as string;
	const override = readHoardSetting<number>("breath.gridIntensity", 0);
	const gridSource = override > 0 ? `custom override (${override} gCO₂/kWh)` : `region "${region}" (${gridIntensity} gCO₂/kWh)`;
	...
	const rateLabel = modelKey === "default"
	...
	const lines: string[] = [
		...
		`    Grid     ${gridSource}`,
		"",
		"  Settings  hoard.breath.{enabled,gridRegion,gridIntensity}",
	];
