...
const DEFAULT_SKIN = readPantrySetting<string>("panels.defaultSkin", "ember");

// Panel-specific keybinds — only active when a panel has focus.
// Read from pantry.panels.keybinds.* in settings, with sane defaults.
const KEYBINDS = {
  close: readPantryKey("panels.keybinds.close", "q"),
  unfocus: readPantryKey("panels.keybinds.unfocus", "escape"),
  scrollUp: readPantryKey("panels.keybinds.scrollUp", "k"),
  scrollDown: readPantryKey("panels.keybinds.scrollDown", "j"),
  skinNext: readPantryKey("panels.keybinds.skinNext", "]"),
  skinPrev: readPantryKey("panels.keybinds.skinPrev", "["),
  // Spatial focus navigation (global — active even without a focused panel)
  focusLeft: readPantryKey("panels.keybinds.focusLeft", "alt+shift+left"),
  focusRight: readPantryKey("panels.keybinds.focusRight", "alt+shift+right"),
  focusUp: readPantryKey("panels.keybinds.focusUp", "alt+shift+up"),
  focusDown: readPantryKey("panels.keybinds.focusDown", "alt+shift+down"),
  // Panel resize (focused panel only)
  resizeWider: readPantryKey("panels.keybinds.resizeWider", "alt+ctrl+right"),
  resizeNarrower: readPantryKey(
    "panels.keybinds.resizeNarrower",
    "alt+ctrl+left",
  ),
  resizeTaller: readPantryKey("panels.keybinds.resizeTaller", "alt+ctrl+down"),
  resizeShorter: readPantryKey("panels.keybinds.resizeShorter", "alt+ctrl+up"),
  // Panel nudge / move position (focused panel only)
  nudgeLeft: readPantryKey("panels.keybinds.nudgeLeft", "ctrl+shift+left"),
  nudgeRight: readPantryKey("panels.keybinds.nudgeRight", "ctrl+shift+right"),
  nudgeUp: readPantryKey("panels.keybinds.nudgeUp", "ctrl+shift+up"),
  nudgeDown: readPantryKey("panels.keybinds.nudgeDown", "ctrl+shift+down"),
};
...
const COLLISION_HEIGHT_CAP_PCT = 0.4;
...
function resolveWidth(width: number | string, termCols: number): number {
  ...
  if (typeof width === "string" && width.endsWith("%")) {
    ...
    return isNaN(pct)
      ? Math.round(termCols * 0.3)
      : Math.round(termCols * (pct / 100));
  }
...
function resolveHeight(
  ...
  if (typeof height === "string" && height.endsWith("%")) {
    ...
    return isNaN(pct)
      ? DEFAULT_ESTIMATED_HEIGHT_ROWS
      : Math.round(termRows * (pct / 100));
  }
...
function computeCellRect(
  ...
  else if (anchor === "center" || anchor.endsWith("center"))
    left = Math.round((termCols - panelW) / 2);
  ...
  for (const anchor of PLACEMENT_PRIORITY) {
    ...
    for (const er of existingRects) {
      const overlapW = Math.max(
        0,
        Math.min(rect.left + rect.width, er.left + er.width) -
          Math.max(rect.left, er.left),
      );
      const overlapH = Math.max(
        0,
        Math.min(rect.top + rect.height, er.top + er.height) -
          Math.max(rect.top, er.top),
      );
...
function adjustForCollision(
  ...
  for (let dy = 1; dy < termRows; dy += 2) {
    ...
    if (
      downRect.top + downRect.height <= termRows &&
      !existingRects.some((er) => rectsOverlap(downRect, er))
    ) {
    ...
    if (
      upRect.top >= 0 &&
      !existingRects.some((er) => rectsOverlap(upRect, er))
    ) {
      return { offsetX: baseOffsetX, offsetY: baseOffsetY - dy };
    }
  }
  ...
  for (let dx = 2; dx < termCols; dx += 2) {
    ...
    if (
      shiftedRect.left + shiftedRect.width <= termCols &&
      !existingRects.some((er) => rectsOverlap(shiftedRect, er))
    ) {
 ...
 */
function suggestLayoutPositions(
  count: number,
  ...
  if (occupiedAnchors.size === 0) {
    if (count === 1)
      return [{ anchor: "right-center", width: "30%", margin: { right: 1 } }];
    if (count === 2)
      return [
  ...
  for (let i = 0; i < count && i < available.length; i++) {
    ...
    const side = anchor.endsWith("right")
      ? "right"
...
class PanelRegistry {
  ...
  isVisibleDuringResize(): boolean {
    return (
      this._resizeVisibleAfter === 0 || Date.now() >= this._resizeVisibleAfter
    );
  }
  ...
  private _sortByDependency(
    items: Array<{ id: string; info: PanelRecreateInfo }>,
  ): Array<{ id: string; info: PanelRecreateInfo }> {
    ...
    for (
      let pass = 0;
      pass < items.length + 1 && remaining.length > 0;
      pass++
    ) {
      ...
      for (const item of remaining) {
        ...
        const depId =
          anchor && typeof anchor === "object" ? anchor.relativeTo : undefined;
  ...
  createPanel(
    ...
    if (this.panels.has(id)) {
      ...
      return {
        success: true,
        message: `Panel '${id}' already open — refreshed`,
      };
    }
    ...
    if (!this._ctx.hasUI) {
      return {
        success: false,
        message: "Error: TUI not available (non-interactive mode)",
      };
    }
    ...
    const panelH = Math.min(
      ...
      Math.max(
        DEFAULT_ESTIMATED_HEIGHT_ROWS,
        Math.round(termRows * COLLISION_HEIGHT_CAP_PCT),
      ),
    );
    ...
    if (opts.anchor === undefined) {
      ...
      if (!refGeo) {
        return {
          success: false,
          message: `Error: reference panel '${anchorRef.relativeTo}' not found`,
        };
      }
      const refRect = this.computeGeoRect(refGeo, termCols, termRows);
      const relRect = computeRelativeRect(
        refGeo,
        refRect,
        panelW,
        panelH,
    ...
    ctx.ui
      .custom(
        (tui, theme, _kb, _done) => {
          ...
          return {
            render: (w: number) =>
              this._suspended ? [] : componentRef!.render(w),
    ...
    return {
      success: true,
      message: `Opened panel '${id}' at ${resolvedAnchor}`,
    };
  }
  ...
  cyclePanelSkin(id: string, direction: 1 | -1 = 1): string {
    ...
    const next =
      skins[
        (((idx + direction) % skins.length) + skins.length) % skins.length
      ]!;
  ...
  focusDirection(direction: "left" | "right" | "up" | "down"): string {
    ...
    if (!this._focusedId || !visible.includes(this._focusedId)) {
      const latest = this._focusHistory
        .slice()
        .reverse()
  ...
  resizePanel(
    ...
    switch (direction) {
      case "wider":
        newOpts.width = Math.min(
          geo.resolvedWidthCols + DEFAULT_RESIZE_STEP,
          termCols - 4,
        );
        break;
      case "narrower":
        newOpts.width = Math.max(
          geo.resolvedWidthCols - DEFAULT_RESIZE_STEP,
          MIN_PANEL_WIDTH,
        );
        break;
      case "taller": {
        const curH =
          typeof info.options.maxHeight === "number"
            ? info.options.maxHeight
      ...
      case "shorter": {
        const curH =
          typeof info.options.maxHeight === "number"
            ? info.options.maxHeight
            : geo.estimatedHeightRows;
        newOpts.maxHeight = Math.max(
          curH - DEFAULT_RESIZE_STEP,
          MIN_PANEL_HEIGHT,
        );
        break;
      }
  ...
  nudgePanel(
    ...
    switch (direction) {
      case "left":
        ...
        break;
      case "right":
        geo.absoluteX = Math.min(
          termCols - geo.resolvedWidthCols - 1,
          (geo.absoluteX ?? 0) + s,
        );
        break;
      case "up":
        ...
        break;
      case "down":
        geo.absoluteY = Math.min(
          termRows - geo.estimatedHeightRows - 1,
          (geo.absoluteY ?? 0) + s,
        );
        break;
    }
  ...
  private _reopenPanel(id: string, refocus: boolean): void {
    ...
    this.createPanel(id, info.factory, {
      ...info.options,
      focusOnOpen: refocus,
    });
  ...
  handlePanelInput(id: string, data: string): boolean {
    ...
    // Resize
    if (matchesKey(data, KEYBINDS.resizeWider)) {
      this.resizePanel(id, "wider");
      return true;
    }
    if (matchesKey(data, KEYBINDS.resizeNarrower)) {
      this.resizePanel(id, "narrower");
      return true;
    }
    if (matchesKey(data, KEYBINDS.resizeTaller)) {
      this.resizePanel(id, "taller");
      return true;
    }
    if (matchesKey(data, KEYBINDS.resizeShorter)) {
      this.resizePanel(id, "shorter");
      return true;
    }
    // Nudge
    if (matchesKey(data, KEYBINDS.nudgeLeft)) {
      this.nudgePanel(id, "left");
      return true;
    }
    if (matchesKey(data, KEYBINDS.nudgeRight)) {
      this.nudgePanel(id, "right");
      return true;
    }
    if (matchesKey(data, KEYBINDS.nudgeUp)) {
      this.nudgePanel(id, "up");
      return true;
    }
    if (matchesKey(data, KEYBINDS.nudgeDown)) {
      this.nudgePanel(id, "down");
      return true;
    }
...
export default function (pi: ExtensionAPI) {
  ...
  const api = {
    ...
    focusDirection: (direction: "left" | "right" | "up" | "down") =>
      registry.focusDirection(direction),
    ...
    handleInput: (id: string, data: string) =>
      registry.handlePanelInput(id, data),
    // Ask prompt coordination — when active, the input listener stands down
    setAskActive: (active: boolean) => {
      registry._askActive = active;
    },
    // Resize + nudge
    resizePanel: (
      id: string,
    ...
    cyclePanelSkin: (id: string, dir?: 1 | -1) =>
      registry.cyclePanelSkin(id, dir),
    ...
    wrapComponent: (panelId: string, inner: any) =>
      registry.wrapComponent(panelId, inner),
    ...
    handler: async (args, ctx) => {
      ...
      switch (subcmd) {
        ...
        case "focus": {
          ...
          ctx.ui.notify(
            target ? registry.focusPanel(target) : registry.focusDirection(dir),
            "info",
          );
          return;
        }
