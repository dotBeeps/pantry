...
function getSetting<T>(key: string, fallback: T): T {
  return readPantrySetting<T>(`imageFetch.${key}`, fallback);
}
...
function isLikelyAI(result: GiphyResult): boolean {
  const haystack =
    `${result.title} ${result.username} ${result.slug}`.toLowerCase();
...
function extractFrames(
  gifBuffer: Buffer,
): { frames: string[]; delays: number[] } | null {
  const dir = join(tmpdir(), `pantry-gif-${Date.now()}`);
  try {
    ...
    try {
      delays = execSync(`magick identify -format "%T\\n" "${gifPath}"`, {
        ...
        .map((d) => {
          ...
          return v > 0 ? v * 10 : DEFAULT_FRAME_DELAY_MS;
        });
    } catch {
      /* use defaults */
    }

    execSync(`magick "${gifPath}" -coalesce "${join(dir, "frame_%04d.png")}"`, {
      timeout: 15000,
    });
    ...
    const frames = files.map((f) =>
      readFileSync(join(dir, f)).toString("base64"),
    );
...
async function bufferToFrames(buf: Buffer): Promise<ImageFrames | null> {
  ...
  const dims = getGifDimensions(buf.toString("base64")) ?? {
    widthPx: 100,
    heightPx: 100,
  };
...
function parseQuery(query: string): {
  source: "giphy" | "tenor" | "url" | "file";
  term: string;
} {
  if (query.startsWith("giphy:"))
    return { source: "giphy", term: query.slice(6).trim() };
  if (query.startsWith("tenor:"))
    return { source: "tenor", term: query.slice(6).trim() };
  if (query.startsWith("http://") || query.startsWith("https://"))
    return { source: "url", term: query };
  if (query.startsWith("/") || query.startsWith("~/"))
    return { source: "file", term: query };
...
function cacheGet(key: string): ImageFrames | undefined {
...
async function fetchImage(
  ...
  if (source === "giphy") {
    ...
    try {
      const resolved = term.startsWith("~/")
        ? join(process.env.HOME ?? "", term.slice(2))
        : term;
      if (existsSync(resolved))
        buf = readFileSync(resolved) as unknown as Buffer;
    } catch {
      return null;
    }
  }
 ...
 */
async function vibeQuery(
  description: string,
  opts: VibeQueryOpts = {},
): Promise<string> {
  if (!getSetting<boolean>("enableVibeQuery", true))
    return getFallbackQuery(opts.tag ?? "");

  const tag = opts.tag ?? "";
  const cacheKey = `${tag}::${description.slice(0, 100)}`;
  const cached = vibeCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < VIBE_CACHE_TTL_MS)
    return cached.query;
