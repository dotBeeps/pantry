...
Centralised image fetching for hoard extensions. Handles Giphy, Tenor, direct URLs, and local files. Includes an optional vibe-query generator that uses an LLM to pick search terms matching a description.
...
```typescript
const query = await imageFetch?.vibeQuery(
    "a list of bug fixes and refactoring tasks",
    { tag: "bugs", extCtx: ctx }
) ?? "bugs";
...
```typescript
pi.on("session_switch" as any, async () => { getImageFetch()?.clearCache(); });
```

## Settings

All under `hoard.imageFetch` in `~/.pi/agent/settings.json`:
...
- **Don't import dragon-image-fetch directly** — use `globalThis[Symbol.for("hoard.imageFetch")]`
