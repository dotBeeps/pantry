/**
 * Integration test for berrygems/extensions/dragon-image-fetch.ts
 *
 * SC-minimum bar per D-04:
 *   1. Extension loads without error via createTestSession.
 *   2. Registers 0 commands and 0 tools.
 *   3. Publication: publishes Symbol.for("pantry.imageFetch") at line 457.
 *
...
const EXT_PATH = path.resolve(
  THIS_DIR,
  "../../extensions/dragon-image-fetch.ts",
);

describe("extension: dragon-image-fetch", () => {
  ...
  it("publishes Symbol.for('pantry.imageFetch') on globalThis", async () => {
    const session = await createTestSession({ extensions: [EXT_PATH] });
    try {
      expect(getGlobal(PANTRY_KEYS.imageFetch)).toBeDefined();
    } finally {
      session.dispose();
    }
  });
