/**
 * Integration test for berrygems/extensions/dragon-breath/
 *
 * SC-minimum bar per D-04:
 *   1. Extension loads without error via createTestSession.
 *   2. Registers 1 command: "carbon". Registers 0 tools.
 *   3. Publication: publishes Symbol.for("pantry.breath") at index.ts:481.
 *
...
const EXT_PATH = path.resolve(
  THIS_DIR,
  "../../extensions/dragon-breath/index.ts",
);

const EXPECTED_COMMANDS = ["carbon"];

describe("extension: dragon-breath", () => {
  ...
  it("publishes Symbol.for('pantry.breath') on globalThis", async () => {
    const session = await createTestSession({ extensions: [EXT_PATH] });
    try {
      expect(getGlobal(PANTRY_KEYS.breath)).toBeDefined();
    } finally {
      session.dispose();
    }
  });
