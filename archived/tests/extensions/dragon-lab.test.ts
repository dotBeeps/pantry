/**
 * Integration test for berrygems/extensions/dragon-lab.ts
 *
 * SC-minimum bar per D-04:
 *   1. Extension loads without error via createTestSession.
 *   2. Registers 0 commands and 0 tools.
 *   3. Publication: publishes Symbol.for("pantry.lab") at line 85.
 *
...
const EXT_PATH = path.resolve(THIS_DIR, "../../extensions/dragon-lab.ts");

describe("extension: dragon-lab", () => {
  ...
  it("publishes Symbol.for('pantry.lab') on globalThis", async () => {
    const session = await createTestSession({ extensions: [EXT_PATH] });
    try {
      expect(getGlobal(PANTRY_KEYS.lab)).toBeDefined();
    } finally {
      session.dispose();
    }
  });
